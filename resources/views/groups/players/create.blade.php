...
@section('title', 'Add Player – ' . $group->name)
...
<div class="max-w-lg mx-auto">
    ...
    <h1 class="text-2xl font-bold text-white mt-2 mb-6">
        <span class="suit suit-club mr-2">♣</span>Add Player
    </h1>

    <div class="card p-6" x-data="{ photoPreview: null }">
        <form method="POST" action="{{ route('players.store', $group) }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex items-center gap-4 mb-6">
                <div class="w-20 h-20 rounded-full overflow-hidden flex items-center justify-center flex-shrink-0"
                    ...
                    <span x-show="!photoPreview" class="text-2xl font-bold" style="color: var(--color-gold);">?</span>
                </div>
                <div>
            ...
            <div class="grid sm:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input id="name" name="name" type="text" class="input @error('name') border-red-500 @enderror"
                        value="{{ old('name') }}" placeholder="John Smith" autofocus>
                    @error('name')<p class="form-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="label" for="nickname">Nickname <span class="text-gray-500">(optional)</span></label>
                    <input id="nickname" name="nickname" type="text" class="input @error('nickname') border-red-500 @enderror"
                        value="{{ old('nickname') }}" placeholder="Ace">
                    @error('nickname')<p class="form-error">{{ $message }}</p>@enderror
                </div>
            </div>

            <div class="mb-4" x-data="{ role: '{{ old('role', 'CORE') }}' }">
            ...
            <div class="mb-6">
                <label class="label" for="email">Email <span class="text-gray-500">(optional)</span></label>
                ...
                <p class="text-xs text-gray-500 mt-1">
                    If provided, they'll receive an invite link to create an account and be linked to this profile.
                    Existing accounts with this email are linked automatically.
                </p>
                @error('email')<p class="form-error">{{ $message }}</p>@enderror
            </div>

            <div class="flex gap-3">
                <button type="submit" class="btn btn-gold">Add to Roster</button>
